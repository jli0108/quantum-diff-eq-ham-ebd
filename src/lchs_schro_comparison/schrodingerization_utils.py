...
def schrodingerization(A, u_0, N, R, T, N_t, lambda_min, r=None):
    ...
    H = 1j * (
        -kron(
            H1,
            (1 / (2 * h)) * diags([np.ones(N - 1), -np.ones(N - 1)], offsets=[1, -1]),
        )
        + 1j * kron(H2, eye(N))
    )
    # Solve the PDE
    ...
    for i, t in enumerate(np.linspace(0, T, N_t)):
        ...
        u_recover[i] = (
            np.exp(a)
            * np.sum(np.reshape(v, newshape=(N_t, dimension, N))[i, :, a_idx:], axis=1)
            * h
        )
...
def schrodingerization_ft(A, u_0, N, R, T, N_t, lambda_min, r=None):
    ...
    p = np.linspace(-R, R, N)
    v_0 = (
        kron(u_0, np.fft.fft(np.exp(-np.abs(p)), norm="ortho")).toarray().flatten()
    )  # second axis is for p
    # v_0 = kron(u_0, np.sqrt(2 / np.pi) / (xi ** 2 + 1)).toarray().flatten() # second axis is for p
    # Write out the Hamiltonian
    H = -(
        kron(H1, -diags(np.fft.fftfreq(N, d=1 / (N * 2 * np.pi / (2 * R)))))
        + kron(H2, eye(N))
    )
    print(H.shape)
    # Solve the PDE
    ...
    for i, t in enumerate(np.linspace(0, T, N_t)):
        ...
        u_recover[i] = (
            np.exp(a)
            * np.sum(
                np.reshape(np.abs(v), newshape=(N_t, dimension, N))[i, :, a_idx:],
                axis=1,
            )
            * h
        )
...
def schrodingerization_ft_trot(A, u_0, N, R, T, N_t, lambda_min, r):
    ...
    xi_pauli_list = SparsePauliOp.from_operator(
        -diags(np.fft.fftfreq(N, d=1 / (N * 2 * np.pi / (2 * R)))).toarray()
    ).to_list()
    ...
    for i in range(len(H1_pauli_list)):
        for j in range(len(xi_pauli_list)):
            pauli_list.append(
                (
                    H1_pauli_list[i][0] + xi_pauli_list[j][0],
                    -H1_pauli_list[i][1] * xi_pauli_list[j][1],
                )
            )
    for i in range(len(H2_pauli_list)):
        pauli_list.append(
            (H2_pauli_list[i][0] + "".join(n_p * ["I"]), -H2_pauli_list[i][1])
        )
    ...
    for t in t_vals:
        ...
        for _ in range(r):
            for i in range(len(pauli_op_commuting)):
                v_t = expm_multiply(
                    -1j * pauli_op_commuting[i].to_matrix() * dt / 2, v_t
                )
...
def get_full_circuit_naive_trotter(n_x, n_p, t, H_1, H_2, r, R, qft_approx_degree=0):
    ...
    state_prep_circuit.append(
        get_qft(n_p, approx_degree=qft_approx_degree).inverse(), qargs=list(range(n_p))
    )
    ...
    for i in range(len(H_1_pauli_list)):
        for j in range(len(xi_pauli_list)):
            pauli_list.append(
                (
                    H_1_pauli_list[i][0] + xi_pauli_list[j][0],
                    -H_1_pauli_list[i][1] * xi_pauli_list[j][1],
                )
            )
    ...
    trot_circuit = SuzukiTrotter(order=2, reps=r).synthesize(
...
def get_full_circuit(
    n_x, n_p, t, H_1, H_2, r, R, trotter_method="second_order", qft_approx_degree=0
):
    N_p = 2**n_p
    h = (2 * R) / (N_p - 1)
    ...
    for _ in range(r):
        if trotter_method == "second_order":
            for pauli_str, coeff in pauli_list_1:
                trot_circuit.append(
                    get_pauli_rotation(pauli_str, 0.5 * dt * coeff.real),
                    qargs=list(range(n_p + n_x)),
                )
            ...
            raise Exception("Trotter method not implemented.")
