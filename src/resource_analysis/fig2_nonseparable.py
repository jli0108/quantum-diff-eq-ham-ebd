...
def get_H_std_binary(N, n_p, R):
    ...
    for j in range(len(H_2_pauli_list)):
        for k in range(len(D_pauli_list)):
            H_pauli_list.append((n_p * 'I' + H_2_pauli_list[j][0] + D_pauli_list[k][0], -H_2_pauli_list[j][1] * D_pauli_list[k][1]))
            H_pauli_list.append((n_p * 'I' + D_pauli_list[k][0] + H_2_pauli_list[j][0], -H_2_pauli_list[j][1] * D_pauli_list[k][1]))
    # Anti-Hermitian part
    for i in range(len(H_F_pauli_list)):
        for j in range(len(H_1_pauli_list)):
            for k in range(len(D_pauli_list)):
                H_pauli_list.append((H_F_pauli_list[i][0] + H_1_pauli_list[j][0] + D_pauli_list[k][0], H_F_pauli_list[i][1] * H_1_pauli_list[j][1] * D_pauli_list[k][1]))
                H_pauli_list.append((H_F_pauli_list[i][0] + D_pauli_list[k][0] + H_1_pauli_list[j][0], H_F_pauli_list[i][1] * H_1_pauli_list[j][1] * D_pauli_list[k][1]))
...
def get_H_one_hot(N, n_p, R):
    ...
    H_1_pauli_list.append((N * 'I', H_1[0,0]))
    for j in range(N):
        op = N * ['I']
        op[j] = 'X'
        op[(j+1)%N] = 'X'
        H_1_pauli_list.append((''.join(op), 0.5*H_1[(j+1)%N,j]))
    ...
    for j in range(N):
        ...
        op[j] = 'X'
        op[(j+1)%N] = 'Y'
        H_2_pauli_list.append((''.join(op), 0.5*H_2[(j+1)%N,j]))
    ...
    for j in range(N):
        x = j / N
        D_pauli_list.append((N * 'I', 0.5 * D[j,j]))
        op = N * ['I']
        op[j] = 'Z'
        D_pauli_list.append((''.join(op), -0.5 * D[j,j]))
