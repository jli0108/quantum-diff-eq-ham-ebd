...
def get_randomized_trotter_error(H, t, r):
    L = len(H)
    lamb = np.max(
        np.abs([np.linalg.norm(H[j].simplify().coeffs, ord=1) for j in range(L)])
    )
    return ((lamb * t * L) ** 4 / (r**3)) * np.exp(2 * lamb * t * L / r) + 2 * (
        (lamb * t * L) ** 3 / (3 * r**2)
    ) * np.exp(lamb * t * L / r)
            ...
            for k in np.arange(j + 1, L):
                for l in np.arange(j + 1, L):
                    error += np.linalg.norm(
                        commutator(H[l], commutator(H[k], H[j])).simplify().coeffs,
                        ord=1,
                    )

                error += 0.5 * np.linalg.norm(
                    commutator(H[j], commutator(H[j], H[k])).simplify().coeffs, ord=1
                )
    ...
    if trotter_method == "first_order":
        for _ in range(r):
            for j in range(len(pauli_op_grouped)):
                ...
                psi_trotter = expm_multiply(
                    -1j * H_j.to_matrix(sparse=True) * dt, psi_trotter
                )
        ...
        for _ in range(r):
            for j in range(len(pauli_op_grouped)):
                H_j = pauli_op_grouped[j]
                psi_trotter = expm_multiply(
                    -1j * H_j.to_matrix(sparse=True) * dt / 2, psi_trotter
                )
...
def std_bin_trotter_error_sampling(
    H, pauli_op, t, r, trotter_method, num_samples, num_jobs
):
    """Uses sampling to compute the Trotter error"""

    res = Parallel(n_jobs=num_jobs)(
        delayed(std_bin_trotter_error_one_sample)(H, pauli_op, t, r, trotter_method)
...
def subspace_trotter_error(A, B, T, r, n, encoding):
    ...
    U_one_trotter_layer = expm(-1j * csc_matrix(A) * (T / r)) @ expm(
        -1j * csc_matrix(B) * (T / r)
    )
...
def get_gate_counts_braket(braket_circuit):
