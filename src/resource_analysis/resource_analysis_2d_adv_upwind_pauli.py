...
if __name__ == "__main__":
    ...
    for i, N in enumerate(N_vals_binary):
        ...
        pauli_basis_circ_depth[i], pauli_basis_two_qubit_gates[i] = (
            pauli_basis_resources_per_trotter_step(
                n_vals_binary[i], n_p, R, T, trotter_method
            )
        )
        pauli_basis_trotter_steps[i] = get_trotter_number_pauli_basis(
            n_vals_binary[i], n_p, R, T, error_tol / dimension, num_samples, num_jobs
        )
