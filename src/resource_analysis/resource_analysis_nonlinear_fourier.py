...
def get_resource_estimate(n_x, n_q, H):
    ...
    circuit.append(LieTrotter(reps=1).synthesize(PauliEvolutionGate(H)), qargs=np.arange(n_q).tolist())
...
def qft_cost(N, encoding):
    if encoding == "std_binary":
        transpiled_qc = transpile(QFT(num_qubits=int(np.log(N))), basis_gates=['rx', 'ry', 'rz', 'rxx'], optimization_level=0)
        ...
        transpiled_qc = transpile(circ, basis_gates=['rx', 'ry', 'rz', 'rxx'], optimization_level=0)
        depth = transpiled_qc.depth(lambda instr: len(instr.qubits) > 1)
        two_qubit_gate_count = transpiled_qc.num_nonlocal_gates()
        # Multiply gate count by two since we consider a 2D problem
        return depth, 2 * two_qubit_gate_count
...
if __name__ == "__main__":
    ...
    for i, N_q in enumerate(N_q_vals):
        ...
        pauli_basis_circ_depth[i], pauli_basis_two_qubit_gates[i] = get_resource_estimate(n_x, int(np.log2(N_q)), H_std_binary)
        ...
        one_hot_circ_depth[i], one_hot_two_qubit_gates[i] = get_resource_estimate(n_x, N_q, H_one_hot)
        ...
        unary_circ_depth[i], unary_two_qubit_gates[i] = get_resource_estimate(n_x, N_q-1, H_unary)
        qft_depth, qft_gates = qft_cost(N_x, encoding="unary")
        unary_circ_depth[i] += 2 * qft_depth
        unary_two_qubit_gates[i] += 2 * qft_gates
        
        np.savez(join("../resource_analysis_data", "nonlinear", f"nonlinear_fourier_data.npz"),
