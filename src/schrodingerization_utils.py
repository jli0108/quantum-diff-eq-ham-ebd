...
def schrodingerization_ft_new_kernel(A, u_0, N, R, T, N_t, lambda_min, r=None, k=1):
    dimension = u_0.shape[0]
    h_p = (2 * R) / (N - 1)
    # Convert ODE to PDE
    H1 = (A + np.conj(A).T) / 2
    ...
    for i in np.arange(1, k+1):
        f[N//2:] += (np.exp(-np.abs(p)) * ((2 ** i / factorial(i)) * p ** i))[N//2:]
    f = np.flip(f)
    # plt.plot(p, f)
    # plt.show()
    v_0 = kron(u_0, np.fft.fft(f, norm="ortho")).toarray().flatten() # second axis is for p
    # v_0 = kron(u_0, np.sqrt(2 / np.pi) / (xi ** 2 + 1)).toarray().flatten() # second axis is for p
    # Write out the Hamiltonian
    H = - (kron(H1, -diags(np.fft.fftfreq(N, d = 1/(N * 2 * np.pi / (2 * R))))) + kron(H2, eye(N)))
    print(H.shape)
    ...
    for i, t in enumerate(np.linspace(0, T, N_t)):
        a = -lambda_min * t
        a_idx = N // 2 + int(a / h_p)
        u_recover[i] = np.exp(a) * np.sum(np.reshape(v, newshape=(N_t, dimension, N))[i,:,a_idx:], axis=1) * h_p
