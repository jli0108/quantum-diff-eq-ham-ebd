...
class advectionSolver():
    def __init__(self, dimension, constList = None, variableCoeffFunc = None, primeFunc = None):
        #super().__init__()
    ...
    def solve(self, discretizeNum, timeLength, provider,trotterNum,trotterOrder,circ = None, onSimulator=True, noise = False):
        ...
        if provider == "ionq":
            if self.circ != None:
                self.iq.compile(self.qs, aais="2pauli",state_prep=self.circ, trotter_num=self.trotterNum,trotter_mode=self.trotterOrder,backend='aria-1')
            else:
                print("-----","Set initial state as 0","-----")
                self.iq.compile(self.qs,aais="2pauli")
            self.iq.run(shots=300, on_simulator=onSimulator, verbose=-1,with_noise=self.noise)
            return self.iq
        ...
        if self.variableCoeffFunc == None:
            if self.dimension == 1:
                ...
                for i in range(self.discretizeNum-1,-1,-1):
                    H -= (self.constList[0]*self.discretizeNum/4) * (self.qubitList[i].Y * self.qubitList[i-1].X - self.qubitList[i].X * self.qubitList[i-1].Y)
            ...
            #    H = self.constList[0] * H
                ...
                H = 0
                for i in range(self.discretizeNum-1,-1,-1):
                    H -= (self.constList[0]*self.discretizeNum/4) * (self.qubitList[i].Y * self.qubitList[(i-1)%self.discretizeNum].X - self.qubitList[i].X * self.qubitList[(i-1)%self.discretizeNum].Y)
                    H -= (self.constList[1]*self.discretizeNum/4) * (self.qubitList[i+self.discretizeNum].Y * self.qubitList[self.discretizeNum+(i-1)%self.discretizeNum].X - self.qubitList[self.discretizeNum+i].X * self.qubitList[self.discretizeNum+(i-1)%self.discretizeNum].Y)
            ...
            if self.dimension == 1:
                ...
                for i in range(self.discretizeNum-1,-1,-1):
                    ...
                    H -= (self.variableCoeffFunc(self.midNodes[inx])*self.discretizeNum/4) * (self.qubitList[i].Y * self.qubitList[i-1].X - self.qubitList[i].X * self.qubitList[i-1].Y)
                ...
                for i in range(self.discretizeNum-1,-1,-1):
                    inx = self.discretizeNum - 1 - i
                    H -= (self.variableCoeffFunc(self.midNodes[inx])*self.discretizeNum/4) * (self.qubitList[i].Y * self.qubitList[(i-1)%self.discretizeNum].X - self.qubitList[i].X * self.qubitList[(i-1)%self.discretizeNum].Y)
                    H -= (self.variableCoeffFunc(self.midNodes[inx])*self.discretizeNum/4) * (self.qubitList[i+self.discretizeNum].Y * self.qubitList[self.discretizeNum+(i-1)%self.discretizeNum].X - self.qubitList[self.discretizeNum+i].X * self.qubitList[self.discretizeNum+(i-1)%self.discretizeNum].Y)
