...
class ionqSP():
    ...
    def statePrepAux(self, startPoint, aList, reverse=False):
        ...
        if len(aList) > 1:
            ...
            if reverse == False:
                ...
                self.partialSWAP(startPoint+half, startPoint+len(aList)-1,np.arccos(np.linalg.norm(aList_L)))
                if len(aList_L) > 1 and np.linalg.norm(aList_L) > 1e-5:
                    self.statePrepAux(startPoint+half+1, aList_L/np.linalg.norm(aList_L))
    ...
    def partialSWAP(self,q0,q1,theta):
        self.circuit = self.circuit.ms(q0,q1,np.pi/2,0,theta).ms(q0,q1,0,-np.pi/2,theta)
    ...
    def split2D(self, aList, reverse=False):
        if reverse:
            ...
            self.circuit = self.circuit.gpi(self.discretizationNum-1,0).gpi(self.sysSize-1,0)
    ...
    def GaussDistribution2D(self):
        self.GHZ([self.discretizationNum//2-1,self.discretizationNum-1,self.discretizationNum + self.discretizationNum//2-1,self.sysSize-1])
        self.circuit = self.circuit.gpi(self.discretizationNum//2-1,0).gpi(self.discretizationNum+self.discretizationNum//2-1,0)

        a1 = gaussAmp(self.discretizationNum,0.25,0.08)[0:self.discretizationNum//2+(self.discretizationNum)%2]
        a2 = gaussAmp(self.discretizationNum,0.75,0.08)[self.discretizationNum//2+(self.discretizationNum)%2:self.discretizationNum]
...
def gaussAmp(n, m, s, double=False):
    ...
    if double:
        ...
        amp = np.array([np.exp(-(((i/n) - m)**2)/(2*s*s)) for i in range(0,n)])
        amp = amp/np.linalg.norm(amp)
