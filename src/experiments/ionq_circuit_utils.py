...
def send_job(job):
    ...
    try:
        ...
        print(req.content)
        raise KeyError(
            f"Error sending job. Error message: {req.content['''message''']}"
        )
    return job_id


def get_ionq_job_json(
    name,
    num_qubits,
    shots,
    device,
...
def cancel_job(job_id):
    ...
    headers = {"Authorization": f"apiKey {IONQ_API_KEY}"}
    r = requests.put(
        f"https://api.ionq.co/v0.3/jobs/{job_id}/status/cancel", headers=headers
    )
    return r
...
def get_ionq_single_job_result(job_id):
    ...
    if status == "completed":
        ...
        req = requests.get(
            f"https://api.ionq.co/v0.3/jobs/{job_id}/results",
            headers=headers,
            params=params,
        )
...
def get_rx(phase, target):
    ...
    return {"gate": "rx", "rotation": phase, "target": target}


def get_ry(phase, target):
    # phase measured in radians
    return {"gate": "ry", "rotation": phase, "target": target}


def get_rz(phase, target):
    # phase measured in radians
    return {"gate": "rz", "rotation": phase, "target": target}


def get_rxx(phase, targets):
    # phase measured in radians
    return {"gate": "xx", "rotation": phase, "targets": targets}


def get_ryy(phase, targets):
    # phase measured in radians
    return {"gate": "yy", "rotation": phase, "targets": targets}


def get_rzz(phase, targets):
    # phase measured in radians
    return {"gate": "zz", "rotation": phase, "targets": targets}


def get_rxy(phase, targets):
    # phase measured in radians
    return {"gate": "xy", "rotation": phase, "targets": targets}


def get_gpi(phase, target):
    # phase measured in turns
    return {"gate": "gpi", "phase": phase, "target": target}


def get_gpi2(phase, target):
    # phase measured in turns
    return {"gate": "gpi2", "phase": phase, "target": target}


def get_ms(phases, angle, targets):
    # assume angle is between 0 and 1
    if 0 <= angle <= 0.25:
        return {"gate": "ms", "phases": phases, "angle": angle, "targets": targets}
    elif 0.75 <= angle <= 1:
        return {
            "gate": "ms",
        ...
        raise ValueError(
            f"Angle is {angle}, must be between 0 and 0.25 or 0.75 and 1 (use two gates instead)"
        )
...
def get_native_circuit(num_qubits, instructions):
    ...
    for op in instructions:
        if op["gate"] == "h":
            ...
            op_list.append(
                get_gpi2((qubit_phase[op["target"]] + 0.25) % 1, op["target"])
            )
            ...
            op_list.append(
                get_gpi2((qubit_phase[op["control"]] + 0.25) % 1, op["control"])
            )

            # XX rotation
            op_list.append(
                get_ms(
                    [qubit_phase[op["control"]], qubit_phase[op["target"]]],
                    0.75,
            ...
            if abs(op["rotation"]) > 1e-5:
                if abs(op["rotation"] / (2 * np.pi) - 0.25) < 1e-6:
                    ...
                    op_list.append(
                        get_gpi2((qubit_phase[op["target"]] + 0.75) % 1, op["target"])
                    )
                elif abs(op["rotation"] / (2 * np.pi) - 0.5) < 1e-6:
                    op_list.append(
                        get_gpi((qubit_phase[op["target"]] + 0.25) % 1, op["target"])
                    )
                elif abs(op["rotation"] / (2 * np.pi) + 0.5) < 1e-6:
                    op_list.append(
                        get_gpi((qubit_phase[op["target"]] + 0.75) % 1, op["target"])
                    )
                else:
                    # Basis change and do virtual Z rotation
                    op_list.append(
                        get_gpi2((qubit_phase[op["target"]] + 0) % 1, op["target"])
                    )
                    qubit_phase[op["target"]] -= op["rotation"] / (2 * np.pi)
                    qubit_phase[op["target"]] %= 1
                    op_list.append(
                        get_gpi2((qubit_phase[op["target"]] + 0.5) % 1, op["target"])
                    )
            ...
            if abs(op["rotation"]) > 1e-5:
                if abs(op["rotation"] / (2 * np.pi) - 0.25) < 1e-6:
                    ...
                    op_list.append(
                        get_gpi((qubit_phase[op["target"]] + 0) % 1, op["target"])
                    )
                elif abs(op["rotation"] / (2 * np.pi) + 0.5) < 1e-6:
                    op_list.append(
                        get_gpi((qubit_phase[op["target"]] + 0.5) % 1, op["target"])
                    )
                else:
                    op_list.append(
                        get_gpi2((qubit_phase[op["target"]] + 0.75) % 1, op["target"])
            ...
            if np.abs(op["rotation"]) > 1e-5:
                if (op["rotation"] / (2 * np.pi)) % 1 <= 0.25 or (
                    op["rotation"] / (2 * np.pi)
                ) % 1 > 0.75:
                    op_list.append(
                        get_ms(
                            [
                                qubit_phase[op["targets"][0]],
                                qubit_phase[op["targets"][1]],
                            ],
                            (op["rotation"] / (2 * np.pi)) % 1,
                            op["targets"],
                        )
                    )
                elif 0.25 < (op["rotation"] / (2 * np.pi)) % 1 <= 0.5:
                    op_list.append(
                        get_gpi(qubit_phase[op["targets"][0]], op["targets"][0])
                    )
                    op_list.append(
                        get_gpi(qubit_phase[op["targets"][1]], op["targets"][1])
                    )
                    op_list.append(
                        get_ms(
                            [
                                (qubit_phase[op["targets"][0]] + 0.5) % 1,
                                qubit_phase[op["targets"][1]],
                            ],
                            0.5 - ((op["rotation"] / (2 * np.pi)) % 1),
                            op["targets"],
                        )
                    )
                    ...
                    op_list.append(
                        get_ms(
                            ...
                            ((op["rotation"] / (2 * np.pi)) % 1) - 0.5,
                            op["targets"],
                        )
                    )
                else:
                    raise ValueError(
                        f"Rotation angle is {op['rotation'] / (2 * np.pi)}, should be between 0 and 1"
                    )
            ...
            if np.abs(op["rotation"]) > 1e-5:
                if (op["rotation"] / (2 * np.pi)) % 1 <= 0.25 or (
                    ...
                    op_list.append(
                        get_ms(
                            [
                                (qubit_phase[op["targets"][0]] + 0.25) % 1,
                                (qubit_phase[op["targets"][1]] + 0.25) % 1,
                            ],
                            (op["rotation"] / (2 * np.pi)) % 1,
                            op["targets"],
                        )
                    ...
                    op_list.append(
                        get_gpi(qubit_phase[op["targets"][0]] + 0.25, op["targets"][0])
                    )
                    op_list.append(
                        get_gpi(qubit_phase[op["targets"][1]] + 0.25, op["targets"][1])
                    )
                    op_list.append(
                        get_ms(
                            [
                                (qubit_phase[op["targets"][0]] + 0.75) % 1,
                                (qubit_phase[op["targets"][1]] + 0.25) % 1,
                            ],
                            0.5 - ((op["rotation"] / (2 * np.pi)) % 1),
                            op["targets"],
            ...
            if np.abs(op["rotation"]) > 1e-5:
                ...
                op_list.append(
                    get_gpi2(
                        (qubit_phase[op["targets"][0]] + 0.5) % 1, op["targets"][0]
                    )
                )
                op_list.append(
                    get_gpi2(
                        (qubit_phase[op["targets"][1]] + 0.5) % 1, op["targets"][1]
                    )
                )
                ...
                # Rotate back
                op_list.append(
                    get_gpi2(qubit_phase[op["targets"][0]], op["targets"][0])
                )
                op_list.append(
                    get_gpi2(qubit_phase[op["targets"][1]], op["targets"][1])
                )

        # phase stored in radians
        elif op["gate"] == "gpi":
            op_list.append(
                get_gpi(qubit_phase[op["target"]] + op["phase"], op["target"])
            )

        elif op["gate"] == "gpi2":
            op_list.append(
                get_gpi2(qubit_phase[op["target"]] + op["phase"], op["target"])
            )
            ...
            op_list.append(
                get_ms(
                    [
                        qubit_phase[op["targets"][0]] + op["phases"][0],
                        qubit_phase[op["targets"][1]] + op["phases"][1],
                    ],
                    op["angle"],
                    op["targets"],
                )
            ...
            raise TypeError(
                f"Gate is {op['''gate''']}, not Rx, Ry, Rz, XX, YY, ZZ, or native"
            )
...
def get_qiskit_circuit(num_qubits, instructions):
    ...
    for op in instructions:
        if op["gate"] == "h":
            ...
            raise TypeError(
                f"Gate is {op['''gate''']}, not H, CNOT, Rx, Ry, Rz, XX, YY, ZZ"
            )
...
def get_circuit_from_qiskit(qiskit_circuit):
    ...
    for item in qiskit_circuit.data:
        ...
        if instruction.name == "rz":
            instructions.append(
                get_rz(
                    instruction.params[0], int(qiskit_circuit.find_bit(qubits[0]).index)
                )
            )
            ...
            instructions.append(
                get_rxx(
                    ...
                    [
                        int(qiskit_circuit.find_bit(qubits[0]).index),
                        int(qiskit_circuit.find_bit(qubits[1]).index),
                    ],
                )
            )
...
def get_circuit_from_braket(braket_circuit):
    ...
    for instruction in braket_circuit.instructions:
        if instruction.operator.name == "H":
            ...
            instructions.append(
                get_cnot(
                    int(instruction.target[0].real), int(instruction.target[1].real)
                )
            )
            ...
            instructions.append(
                get_rz(instruction.operator.angle, int(instruction.target[0].real))
            )

        elif instruction.operator.name == "Ry":
            instructions.append(
                get_ry(instruction.operator.angle, int(instruction.target[0].real))
            )

        elif instruction.operator.name == "Rx":
            instructions.append(
                get_rx(instruction.operator.angle, int(instruction.target[0].real))
            )

        elif instruction.operator.name == "XX":
            instructions.append(
                get_rxx(
                    instruction.operator.angle,
                    [int(instruction.target[0].real), int(instruction.target[1].real)],
                )
            )
            ...
            raise TypeError(
                f"Gate is {instruction.operator.name}, not Rx, Ry, Rz, XX, YY"
            )
    return instructions
...
def state_prep_circuit(N, dimension, amplitudes, encoding):
    ...
    for i in range(dimension):
        ...
        if encoding == "unary" or encoding == "antiferromagnetic":
            assert np.all(np.isreal(amplitudes)) and np.all(
                amplitudes >= 0
            ), "Only supports positive real valued amplitudes"
            ...
            for k in np.arange(0, n - 1):
                if np.linalg.norm(amplitudes[k + 1 :]) > 0:
                    ...
                    instructions.append(
                        get_ryy(-np.arccos(a), [i * n + k, i * n + k + 1])
                    )
                    instructions.append(get_rx(0.25 * (2 * np.pi), i * n + k))
...
def state_prep_one_hot_aux(n, starting_index, amplitudes):
    ...
    if n > 1:
        ...
        if np.linalg.norm(amplitudes_right) > 0:
            instructions.append(
                get_rxy(np.arccos(a), [starting_index, starting_index + int(n / 2)])
            )
            instructions.append(
                get_rxy(-np.arccos(a), [starting_index + int(n / 2), starting_index])
            )

        if np.linalg.norm(amplitudes_left) > 0:
            instructions += state_prep_one_hot_aux(
                int(n / 2),
                starting_index,
