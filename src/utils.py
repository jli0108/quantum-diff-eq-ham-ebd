...
def sum_x(n: int) -> np.ndarray:
    ...
    for i in range(n):
        res += tensor(
            [
                identity(dims[n - i - 1], format="csr"),
                PAULI_X,
                identity(dims[i], format="csr"),
            ]
        )
        ...
        for j in range(i):
            res += V[i, j] * tensor(
                [
                    identity(dims[n - i - 1], format="csr"),
                    NUMBER,
                    identity(dims[i - j - 1], format="csr"),
                    NUMBER,
                    identity(dims[j], format="csr"),
                ]
            )
...
def num_qubits_per_dim(N, encoding, periodic=False):
    if encoding == "one-hot":
        ...
        raise ValueError(
            "Encoding not supported. Valid encodings: unary, antiferromagnetic, one-hot"
        )
